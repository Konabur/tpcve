...
NAME = "percentile"
COLUMNS = ["file", *C.LABEL_COLS, "percentile", "n_veg", "h_p", "error"]
...
def run_analyze(argv=None) -> int:
    ...
    return C.run_long_analyze(args, value_cols=["h_p"],
                              group_cols=["percentile"],
                              label_fn=lambda meta, vc: f"p{float(meta['percentile']):g}",
                              kind_regression="regression_height",
                              kind_plots="regression_plots_height")
