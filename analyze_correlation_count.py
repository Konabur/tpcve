"""Регрессия biomass ~ n_points для long-формата CSV из batch_count.py.

Группирует строки по (source) ∈ {"raw", "pre"}. Для каждой группы фитит
linear / power / huber и выбирает лучшую модель по R².

Использование:
    uv run python analyze_correlation_count.py results/volume_csv/count/<name>.csv
    uv run python analyze_correlation_count.py results/volume_csv/count/<name>.csv \\
        --plots-dir
"""
...
def main() -> int:
    ...
    p.add_argument("csv", help="CSV из batch_count.py")
    ...
    p.add_argument("--output", default=None,
                   help="Куда сохранить CSV с результатами регрессии "
                        "(default: results/regression_csv/count/<stem>_regression.csv)")
    ...
    if "n_points" not in df.columns or "source" not in df.columns:
        raise SystemExit("Нет колонок n_points/source — это не batch_count CSV?")
    ...
    for keys, grp in df.groupby(GROUP_COLS, dropna=False):
        ...
        s = pd.to_numeric(grp["n_points"], errors="coerce")
    ...
    print(f"\nЦель: {args.target} ~ n_points   (групп: {len(res_df)})")
    print("=" * 140)
    ...
    if args.output:
        ...
        out_csv = default_path("regression_count", stem + "_regression", ".csv")
    ...
    if df_test is not None:
        ...
        for r in res_df.itertuples():
            ...
            if result is None:
                continue
            grp = test_groups.get((r.source,))
            if grp is None or "n_points" not in grp.columns:
                continue
            s = pd.to_numeric(grp["n_points"], errors="coerce")
            y = pd.to_numeric(grp[args.target], errors="coerce")
            mask = s.notna() & y.notna() & (s > 0)
    ...
    if args.plots_dir:
        ...
        if args.plots_dir == "__auto__":
            out = default_path("regression_plots_count", stem, ext="")
        else:
        ...
        for r in res_df.itertuples():
            ...
            plot_fits(ax, x, yv, result,
                      xlabel="n_points",
                      ylabel=args.target,
                      title=f"{args.target} ~ n_points ({r.source})",
